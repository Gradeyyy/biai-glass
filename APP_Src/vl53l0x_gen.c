...
VL53L0X_Error vl53l0x_set_mode(VL53L0X_Dev_t *dev, uint8_t mode) {
	...
	status = VL53L0X_SetLimitCheckEnable(dev,
			VL53L0X_CHECKENABLE_SIGMA_FINAL_RANGE, 1); //使能SIGMA范围检查
	if (status != VL53L0X_ERROR_NONE)
		goto error;
	HAL_Delay(2);
	status = VL53L0X_SetLimitCheckEnable(dev,
	...
	status = VL53L0X_SetLimitCheckValue(dev,
			VL53L0X_CHECKENABLE_SIGMA_FINAL_RANGE, Mode_data[mode].sigmaLimit); //设定SIGMA范围
	if (status != VL53L0X_ERROR_NONE)
		goto error;
	HAL_Delay(2);
	status = VL53L0X_SetLimitCheckValue(dev,
	...
	status = VL53L0X_SetVcselPulsePeriod(dev, VL53L0X_VCSEL_PERIOD_PRE_RANGE,
			Mode_data[mode].preRangeVcselPeriod); //设定VCSEL脉冲周期
	if (status != VL53L0X_ERROR_NONE)
		goto error;
	HAL_Delay(2);
	status = VL53L0X_SetVcselPulsePeriod(dev, VL53L0X_VCSEL_PERIOD_FINAL_RANGE,
			Mode_data[mode].finalRangeVcselPeriod); //设定VCSEL脉冲周期范围
...
//		key = KEY_Scan(0);
		...
		else if (key == KEY1_PRES) //选择工作模式
		{
			...
			switch (mode) {
			case Default_Mode:
//				LCD_ShowString(95, 140 + 95, 300, 16, 16, "Default        ");
				break; //默认
			case HIGH_ACCURACY:
//				LCD_ShowString(95, 140 + 95, 300, 16, 16, "High Accuracy  ");
				break; //高精度
			case LONG_RANGE:
//				LCD_ShowString(95, 140 + 95, 300, 16, 16, "Long Range     ");
				break; //长距离
			case HIGH_SPEED:
//				LCD_ShowString(95, 140 + 95, 300, 16, 16, "High Speed     ");
				break; //高速
			}
